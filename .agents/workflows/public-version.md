...
// cd frontend && npm run build && cd ../desktop && npm run build && cd .. && cp "desktop/dist-standalone/nsis-web/GuildManagerSetup.exe" backend/updates/ && cp "desktop/dist-standalone/nsis-web/*.nsis.7z" backend/updates/ && cp "desktop/dist-standalone/nsis-web/latest.yml" backend/updates/ && git add . && git commit -m "[RELEASE] New Public Version for Railway" && git push origin main
